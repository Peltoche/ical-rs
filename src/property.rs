...
impl Type {
    ...
    pub fn from_str(input: &str) -> Result<Type, ParseError> {
        match input.to_lowercase().as_str() {
            ...
            "xml" => Ok(Type::Xml),
            "label" => Ok(Type::Label),
            "mailer" => Ok(Type::Mailer),
            "class" => Ok(Type::Class),
            "agent" => Ok(Type::Agent),
            "prodid" => Ok(Type::ProdId),
            "categories" => Ok(Type::Categories),
            "sort-string" => Ok(Type::SortString),
...
pub fn get_vcard_design(version: Version) -> Design {
    let mut design: Design;

    design = get_vcard_common();

    match version {
        Version::Three => add_vcard3_properties(&mut design),
        Version::Four => add_vcard4_properties(&mut design),
    }

    design
}

fn get_vcard_common() -> Design {
    ...
    v_design.insert(Type::Nickname,
                    DesignElem {
                        value_type: value::Type::TextMultiComa,
                        allowed_types: None,
                    });
...
fn add_vcard3_properties(design: &mut Design) {
    design.insert(Type::Label, DEFAULT_TYPE_TEXT);
    design.insert(Type::Mailer, DEFAULT_TYPE_TEXT);
    design.insert(Type::Class, DEFAULT_TYPE_TEXT);
    design.insert(Type::Agent, DEFAULT_TYPE_URI);
    design.insert(Type::Categories,
                  DesignElem {
                      value_type: value::Type::TextMultiComa,
                      allowed_types: None,
                  });
    design.insert(Type::ProdId, DEFAULT_TYPE_TEXT);
    design.insert(Type::SortString, DEFAULT_TYPE_TEXT);
}

fn add_vcard4_properties(design: &mut Design) {
    design.insert(Type::Kind, DEFAULT_TYPE_TEXT);
    design.insert(Type::Gender, DEFAULT_TYPE_TEXT_MULTI);
    design.insert(Type::Lang, DEFAULT_TYPE_TEXT);
    design.insert(Type::Anniversary,
                  DesignElem {
                      value_type: value::Type::DateAndOrTime,
                      allowed_types: Some(vec![value::Type::DateTime,
                                               value::Type::Date,
    ...
    design.insert(Type::Xml, DEFAULT_TYPE_TEXT);
    design.insert(Type::Clientpidmap, DEFAULT_TYPE_TEXT_MULTI);

    // RFC2739
    design.insert(Type::Fburl, DEFAULT_TYPE_URI);
    design.insert(Type::Caladruri, DEFAULT_TYPE_URI);
    design.insert(Type::Caluri, DEFAULT_TYPE_URI);

    // RFC4770
    design.insert(Type::Impp, DEFAULT_TYPE_URI);
}
